...
use Charts;
class GetAverageController extends Controller
{
    
    public function index()
    {
        return view('admin.feedbacks.get');
    }
...
//    dd( $sanitationAverage);
    ...
    //     ->whereRaw(" restaurantID='$v' ")->get()->pluck('count(id)');
      ...
      $chart = Charts::database(Feedback::all(), 'bar', 'highcharts')
        ...
        $chart1 = Charts::database(Feedback::all(), 'bar', 'highcharts')
        ->title("Feedback Details")
        ->elementLabel("Total Feedbacks")
        ->values(Feedback::all())
        ->dimensions(1000, 500)
        ...
        return view('admin.averages.getavg',['feedback'=>$feedbacks, 'avgStar'=>$avgStar ,
